...
function MarcaPage() {
  ...
  const cargar = useCallback(() => {
    ...
    getProductos({ marca: decodedNombre, page, limit })
      .then(r => {
        const raw = r.data
  ...
  const color      = getBrandColor(decodedNombre)
  const initials   = decodedNombre.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2)

  return (
    <main>
      <Breadcrumb items={[{ label: 'Marcas', path: '/marcas' }]} current={decodedNombre} />
        ...
        <div className="container">
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.75rem', color, letterSpacing: '-0.5px' }}>
                {initials}
              </span>
            </div>
            <div style={{ flex: 1 }}>
              <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', fontWeight: 700, color: 'var(--fg)', margin: 0, lineHeight: 1.1 }}>
                {decodedNombre}
              </h1>
              <p style={{ color: 'var(--subtle)', marginTop: '0.4rem', fontSize: '0.9rem' }}>
            ...
            {loading ? (
              ...
              <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '5rem 2rem' }}>
                ...
                <p style={{ color: 'var(--subtle)', fontSize: '0.875rem', marginBottom: '1.25rem' }}>
                  No hay productos registrados para <strong>{decodedNombre}</strong>
                </p>
                <button onClick={() => navigate('/marcas')} style={{ padding: '0.65rem 1.25rem', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: 'pointer' }}>
                  Ver otras marcas
                </button>
              </div>
            ) : (
